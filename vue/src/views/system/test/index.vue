    ...
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="testList" @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55" align="center"/>
                                                                                <el-table-column label="商品ID" align="center" prop="test"/>
...
<script>
  import {addTest, delTest, getTest, listTest, updateTest} from "@/api/system/test";
  ...
  export default {
        ...
        methods: {
            ...
            handleSelectionChange(selection) {
                this.ids = selection.map(item => item.test)
            ...
            /** 删除按钮操作 */
            handleDelete(row) {
                const tests = row.test || this.ids;
                this.$confirm('是否确认删除【请填写功能名称】编号为"' + tests + '"的数据项?', "警告", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                }).then(function () {
                    return delTest(tests);
                }).then(() => {
                    this.getList();
